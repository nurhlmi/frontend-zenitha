...
export default function OrderDetail(props) {
   ...
   const CustomStepIcon = (index) => {
      return (
         <Typography color={index !== 0 ? "text.secondary" : "#000"}>
            <Circle />
         </Typography>
      );
   };
   ...
   return (
      <Container sx={{ flex: 1 }}>
                           ...
                           <CardContent>
                              <Box sx={{ borderBottom: "1px dashed #e0e0e0", pb: 2, mb: 2 }}>
                                 <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    ...
                                    {data.status === "sent" && data.number_resi !== null && (
                                       <Button variant="outlined" onClick={handleTrackingDialog}>
                                          Lacak
                                       </Button>
                                    )}
                                 </Box>
                                    ...
                                    data.payment.map((value, index) => (
                                       <Box sx={{ display: "flex", justifyContent: "space-between", mt: 1 }} key={index}>
                                          <Typography variant="body2" color="text.secondary">
                                             Pembayaran {data.payment.length > 1 && `ke-${index + 1}`}
                                          </Typography>
                        ...
                        <Card sx={{ mb: 2 }}>
                           <CardContent>
                              ...
                              {data.transaction_product.map((value, index) => (
                                 <Box sx={{ mb: data.transaction_product.length !== index + 1 ? 2 : 0 }} key={index}>
                                    <Grid container>
                                       <Grid item>
         ...
         <Dialog open={trackingDialog} onClose={handleTrackingDialog} maxWidth="xs" fullWidth>
